...
class App extends Container
{
    ...
    public function init($module = '')
    {
        ...
        if (is_file($path . 'XinConst.php')) {
            include_once $path . 'XinConst.php';
        }
    ...
    public function routeCheck()
    {
        ...
        if(count($patharray)==3 && $patharray[0]=='api'){
            $ONE_appname = $patharray[1];
            $ONE_appaction = $patharray[2];
            $apps = '\app\api\controller\\'.ucfirst($ONE_appname);
            if(!class_exists($apps))
            {
                $plugins = '\app\plugins\\'.$ONE_appname.'\\'.ucfirst('api');
                if(!class_exists($plugins))
                {
                    show_json(1001, ucfirst($ONE_appname).' 应用控制器未定义');
                }else{
                    $path = 'index/plugins/index/pluginsname/'.$ONE_appname.'/pluginscontrol/api/pluginsaction/'.$ONE_appaction;
                }
            }
        }
