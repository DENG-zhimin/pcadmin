...
class Email
{
	...
	{
		$this->interval_time = isset($param['interval_time']) ? intval($param['interval_time']) : 90;
		$this->expire_time = isset($param['expire_time']) ? intval($param['expire_time']) : 90;
	...
	private function EmailInit()
	{
		...
		$this->obj->SMTPDebug = 4;
//		$this->obj->Port = '465';
//		$this->obj->SMTPSecure = 'ssl';
//		$this->obj->SMTPOptions = array(
//			'ssl' => array(
//		    'verify_peer' => false,
...
//		);
			...
			//仅查找ipv4地址
//		$this->obj->Host = gethostbyname('smtp.163.com');
		// 邮箱用户名
	...
	public function SendHtml($param = array())
	{
		...
		if($this->obj->Send())
		{
			...
			if(!empty($param['code']))
			{
				$a =$this->KindofSession($param['code']);
			}
			return true;
		} else {
	...
	{
		if(session($this->key_code))
		{
			$data = session($this->key_code);
	...
	{
		if(session($this->key_code)['code'])
		{
			if(empty($code) && isset($_POST['code']))
			{
				$code = trim($_POST['code']);
			}
			return (session($this->key_code)['code'] == $code);
		}
		return false;
	}
