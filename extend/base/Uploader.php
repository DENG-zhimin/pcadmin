...
class Uploader
{
    ...
    * @version 1.0.0
    ...
    {
        //处理附件资料
        $root_path = substr(ROOT,0,strlen(ROOT)-1);
        $content = $filename = $root_path.$this->fullName;
        //echo $content;exit;
        $md5 = md5($filename);
        $filename = date('Y').'/'.date('m').'/'.date('d') . '/'.$md5.'.' . pathinfo($filename, 4);
        // 检查文件是否已上传
        if (($site_url = FileService::getFileUrl($filename))) {
            $this->result(['site_url' => $site_url], 'IS_FOUND');
        }
        $uptype = MyC('common_storage_type');

        // 需要上传文件，生成上传配置参数
        $result = ['uptype' => $uptype, 'file_url' => $filename];
        $fullname = $this->fullName;
        switch (strtolower($uptype)) {
            case 'qiniu':
                $res = FileService::qiniuRemote($filename, $content);
                //print_r($res);exit;
                $fullname = $res['url'];
                break;
        }
